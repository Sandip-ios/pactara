    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          // ONLY an exact IP match, within a short window. Looser matching
          // (network prefix, or "any recent invite for this platform") made
          // strangers behind the same carrier NAT inherit each other's
          // invites — people saw invites nobody sent them.
          const { data } = await supabaseAdmin
            ...
            .is("claimed_at", null)
            .eq("ip_hash", ipHash)
            .gte("created_at", windowStartIso(30))
